...
window, a workday sensor, a Jinja template sensor with a nested `if` chain, a
`utility_meter` with tariffs, and an automation for every window boundary — then
...
**It decides nothing.** No optimiser, no load scheduling, no "cheapest window"
...
| `sensor.<name>_allowance_remaining` | kWh left of a capped free window, when you have one |
...
that touches windows, so a gap or an overlap is visible the moment you make it:
...
Every day
  ...
  Coverage: complete. 6 windows, no gaps, no overlaps.
```

### Rates first

**A rate is defined once and pointed at by any number of windows.** Change the
...
| Demand charge window | A flag only. The $/kW/month arithmetic is out of scope |
| Daily energy allowance | Leave empty for none. Some plans give a window free only up to a cap |
| Rate beyond the allowance | Required when there is an allowance |

### Then windows

Three fields: start, end, rate. Start is included, end is not. A window running
to midnight ends at 00:00.

The windows in a day set must cover 00:00 to 24:00 exactly once. The plan will
...
another — build the weekday set, then use **Duplicate day set** and change the
rates on the windows that differ.
...
A day set can carry a date range, entered as `MM-DD`. A range may wrap the new
...
supply charge pair** under Utility meters and you get two entities: an
...
Nominate that select under Utility meters and the integration sets it at each
window boundary — the automation people normally write for this disappears.
...
The next window boundary is computed and scheduled; when it fires, every entity
...
Windows are wall-clock times, which is what you want: a peak window is 16:00
...
**The price sensor is unavailable.** No window resolves at this moment. Open
Configure and read the strip — there is a gap, or a day type that no day set
covers.
...
is nominated under Utility meters; the tariff names on the meter exactly match
...
- **Demand charges are a flag only.** A window can be marked as one so a
...
- **No text or CSV import.** With rates defined once, a window is three fields.
...
Anything expressible as named rates over non-overlapping windows:
...
- Free or discounted windows, with or without a daily energy cap
